...
- **Persistence** — `isSpire`/`spireClass` are saved + restored (`saved-run.ts`, `run-save.ts` `restoreRunToState`). Run progression randomness is counter-based (`runRngSeed` + `runRngCounter` on `GameState`; `withRunRng()` scopes map, encounter, reward, shop, event, and between-stage rolls without changing combat RNG). Saves also retain the exact pending encounter/snapshot and Elite Design reward source, approved Elite Design assignments keyed by map-node id, unclaimed PokeMart offers, and unresolved rest/event choices. `stopSpireFightingPhase()` runs the between-stage transaction synchronously after battle resolution and before the reward-phase save; `postBattleEffectsNodeId` makes that transaction exactly-once across phase retries/resume. `initializePickingPhase()` only bootstraps those effects before the first fight, then post-battle processing prepares every later fight. Permanent-stat stacking policy was intentionally left unchanged pending a separate balance decision.
...
- Gold Bottle Cap (`items.ts`): Crit power bonus capped at 200 gold (was uncapped); grants 1 gold per KO up to 3 gold per round, with no last-enemy jackpot
...
**Pokemon:**
- Snorlax/Munchlax (`Passive.GLUTTON`): Permanently gains 5 max HP per berry, 5 per dish, and 10 when cooking as a Chef (upstream: 10/20/30)
- Ralts/Kirlia/Gardevoir (`Ability.FUTURE_SIGHT`): Still selects up to 5 random enemies and deals damage after 2 seconds, but no longer deals 20% splash damage to adjacent enemies. This prevents the five marked targets from chaining damage across most or all of the board.
...
- Happiny/Chansey/Blissey (`Ability.SOFT_BOILED`): Team-wide shield AP scaling reduced from 1 to 0.5; flat 20/40/80 shield values and status cleanse unchanged
- Pawniard/Bisharp/Kingambit (`pokemon.ts`): `maxPP` 60→80 on all three stages; synced in `precomputed/pokemons-data.csv`
- Shinx/Luxio/Luxray (`pokemon.ts`): `maxPP` 70→80 on all three stages; synced in `precomputed/pokemons-data.csv`
...
- Fuecoco/Crocalor/Skeledirge (`Ability.TORCH_SONG`): Flame count hard-capped at 20 (`Math.min(20, …)`), and each flame's raw SPECIAL damage is capped at 70 (`Math.min(70, 0.5 * pokemon.atk)`). The AP buff still applies **once per flame** (as upstream does) — capping the flame count is what bounds the command queue, so per-flame AP can no longer run away. Without the count cap this was a feedback loop (AP-scaled flame count + per-flame AP gain) that flooded `pokemon.commands` with unbounded `DelayedCommand`s, leaking memory and OOM-crashing the production server. The English tooltip includes the damage cap; other locales retain the upstream formula.
