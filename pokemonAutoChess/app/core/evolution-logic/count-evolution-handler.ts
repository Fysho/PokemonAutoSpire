...
export class CountEvolutionHandler extends EvolutionHandler {
  ...
  canEvolve(pokemon: Pokemon, player: Player): boolean {
    ...
    // Spire: 2★+ units only need min(numberRequired, 2) copies — extra copies of a
    // high-tier unit are much harder to find in single-player. See 6.9→6.10 migration doc.
    const required =
      pokemon.stars >= 2 ? Math.min(this.numberRequired, 2) : this.numberRequired
  ...
  canEvolveIfGettingOne(pokemon: Pokemon, player: Player): boolean {
    ...
    // Spire: mirror the 2★+ reduced-copy rule from canEvolve. See 6.9→6.10 migration doc.
    const required =
      pokemon.stars >= 2 ? Math.min(this.numberRequired, 2) : this.numberRequired
  ...
  evolve(pokemon: Pokemon, player: Player): Pokemon {
    ...
    // Spire: consume only `required` copies (2★+ reduced-copy rule). See 6.9→6.10 migration doc.
    const required =
      pokemon.stars >= 2 ? Math.min(this.numberRequired, 2) : this.numberRequired
