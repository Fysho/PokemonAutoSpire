...
export class Tandemaus extends Pokemon {
  ...
  // PAC diversion: upstream evolves on fixed stageLevel >= 14, unreachable if acquired
  // late. Spire evolves 5 fights after acquisition via a hatch-style timer (hatchTime),
  // replacing the old TimerEvolutionRule class. See 6.9→6.10 migration doc.
  evolutionRule = {
...
export class MausholdThree extends Pokemon {
  ...
  // PAC diversion: upstream evolves on fixed stageLevel >= 20. Spire evolves 5 more
  // fights after Maushold (3) appears (10 total from acquiring Tandemaus) via a
  // hatch-style timer, replacing the old TimerEvolutionRule. See 6.9→6.10 migration doc.
  evolutionRule = {
    type: EvolutionRuleType.HATCH,
    hatchTime: 5
  } satisfies HatchEvolutionRule
...
export class Cosmog extends Pokemon {
  ...
  evolutionRule = { type: EvolutionRuleType.STACK } satisfies StackEvolutionRule
  stacksRequired = 3 // PAC diversion: was 8 (10 upstream), lowered so Cosmog evolves on a realistic number of evolutions in a Spire run. See 6.9→6.10 migration doc.
  stars = 1
  hp = 140
  atk = 5
  speed = 37
...
export class Cosmoem extends Pokemon {
  ...
  stacksRequired = 3 // PAC diversion: was 8 (10 upstream). See 6.9→6.10 migration doc.
  onAcquired(player: Player) {
